...
class AnalysisTests(unittest.TestCase):
    ...
    def test_categorical_charts_use_single_line_truncated_labels_with_full_values(self):
        ...
        self.assertGreaterEqual(len(fig.layout.xaxis.ticktext[1].rstrip("…")), 33)

    def test_distribution_chart_units_and_data_labels_are_explicit(self):
        df = pd.DataFrame({"primary_theme": ["A", "A", "B"], "theme": ["A", "B", "B"]})
        responses = theme_bar_chart(df, "primary_theme", total=4, unit="Responses")
        tags = theme_bar_chart(df, "theme", total=3, unit="Theme Tags")
        self.assertEqual(responses.layout.xaxis.title.text, "Number of Responses")
        self.assertEqual(tags.layout.xaxis.title.text, "Number of Theme Tags")
        self.assertIn("2 (50.0%)", list(responses.data[0].text))
    ...
    def test_valence_charts_share_full_ordered_y_axis_labels(self):
        ...
        self.assertEqual(overall.layout.xaxis.title.text, "Number of Responses")
        self.assertIsNone(overall.layout.yaxis.title.text)
        self.assertEqual(list(overall.data[0].text), ["0 (0.0%)", "1 (50.0%)", "0 (0.0%)", "1 (50.0%)", "0 (0.0%)"])
    ...
    def test_emotion_overall_places_angry_at_bottom(self):
        ...
        self.assertEqual(fig.layout.xaxis.title.text, "Number of Responses")
        self.assertEqual(list(fig.data[0].text), ["1 (33.3%)", "1 (33.3%)", "1 (33.3%)"])
