...
def generate_notebook(
        ...
        for cov in covariate_cols:
            ...
            if cov_type == "categorical":
                try:
                    ...
                    stat_note = (f'{stats["method"]}: statistic = {stats["chi2"]}, '
                                 f'p = {p_text} ({sig}), bias-corrected Cramér\'s V = '
                                 f'{stats["cramers_v"]}. Expected-count diagnostic: minimum '
                                 f'{stats["expected_min"]}; {stats["expected_below_5_pct"]}% below 5.')
                except Exception:
                    stat_note = ""
                ...
                cells.append(_code(
                    ...
                    f"print(f\"{{_assoc['method']}}: statistic={{_assoc['chi2']:.3f}}, "
                    f"p={{_p_text}}, df={{_assoc['dof']}}, bias-corrected Cramér's V={{_assoc['cramers_v']:.3f}}\")\n"
                    f"print(f\"Expected-count diagnostic: minimum={{_assoc['expected_min']:.3f}}; "
                    f"{{_assoc['expected_below_5_pct']:.1f}}% below 5\")"
                ))
                ...
                cells.append(_code(
                    ...
                    f"    print(f\"Welch's ANOVA: F={{_f:.3f}}, df={{_df1}},{{_df2:.1f}}, p={{_pt}}, descriptive η²={{_eta2}}\")\n"
                    f"    if _p < 0.05:\n"
                    f"        _pairs = list(itertools.combinations(sorted(_groups), 2))\n"
                    f"        _rows = []\n"
                    f"        for a, b in _pairs:\n"
    ...
    if include_valence and "valence_score" in coded_df.columns:
        ...
        cells.append(_code(
            ...
            "    print(f\"Welch's ANOVA: F={_f:.3f}, df={_df1},{_df2:.1f}, p={_pt}, descriptive η²={_eta2}\")\n"
            "    if _p < 0.05:\n"
            "        _pairs = list(itertools.combinations(sorted(_vg), 2))\n"
            "        _rows = []\n"
            "        for a, b in _pairs:\n"
    ...
    if include_emotion and "emotion" in coded_df.columns:
        ...
        cells.append(_code(
            ...
            "print(f\"{_emo_assoc['method']}: statistic={_emo_assoc['chi2']:.3f}, "
            "df={_emo_assoc['dof']}, p={_emo_p}, bias-corrected Cramér's V={_emo_assoc['cramers_v']:.3f}\")\n"
            "print(f\"Expected-count diagnostic: minimum={_emo_assoc['expected_min']:.3f}; "
            "{_emo_assoc['expected_below_5_pct']:.1f}% below 5\")\n"
    ...
    cells.append(_md(
        ...
        "the notebook reports a deterministic permutation p-value instead and prints the diagnostic.\n\n"
