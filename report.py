...
def generate_notebook(
        ...
        for cov in covariate_cols:
            ...
            if cov_type == "categorical":
                ...
                cells.append(_code(
                    ...
                    f"        _ph['Below α=.05'] = _ph['p-adj (Bonferroni)'].apply(lambda p: '✓' if p < 0.05 else '')\n"
                    f"        display(_ph)\n"
                    f"else:\n"
    ...
    if include_valence and "valence_score" in coded_df.columns:
        ...
        cells.append(_md(
            ...
            f"The Bonferroni-adjusted threshold is α = {supplemental_alpha:.3f} (familywise α = 0.05 "
        ...
        cells.append(_code(
            ...
            f"    print(f'Kruskal-Wallis: H={{_h:.2f}}, {{_pc}}, ε²={{_eps2:.3f}}; threshold α = {supplemental_alpha:.3f}')\n"
            ...
            "        _ph['Below α=.05'] = _ph['p-adj (Bonferroni)'].apply(lambda p: '✓' if p < 0.05 else '')\n"
            "        display(_ph)\n"
            "else:\n"
            ...
            "    _pt = '<0.001' if _p < 0.001 else f'{_p:.3f}'\n"
            "    _pc = f'p {_pt}' if _pt.startswith('<') else f'p = {_pt}'\n"
            f"    print(f\"Welch's ANOVA: F={{_f:.2f}}, df={{_df1}},{{_df2:.1f}}, {{_pc}}, descriptive η²={{_eta2:.3f}}; threshold α = {supplemental_alpha:.3f}\")\n"
            f"    if _p < {supplemental_alpha}:\n"
            "        _pairs = list(itertools.combinations(sorted(_vg), 2))\n"
            "        _rows = []\n"
            "        for a, b in _pairs:\n"
    ...
    if include_emotion and "emotion" in coded_df.columns:
        ...
        cells.append(_code(
            ...
            f"print('This p-value is below threshold α = {supplemental_alpha:.3f}.' "
            f"if _emo_assoc['p_value'] < {supplemental_alpha} else "
            f"'This p-value is not below threshold α = {supplemental_alpha:.3f}.')\n"
