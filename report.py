...
def _style(fig):
    ...
    fig.update_xaxes(tickfont_size=15, title_font_size=18, automargin=True)
    fig.update_yaxes(tickfont_size=15, title_font_size=18, automargin=True)
    return fig
"""

_WRAP_FN = """\
def _format_category_label(text, max_chars=40):
...
def generate_notebook(
    ...
    theme_distribution_view: str = "all_tags",
) -> bytes:
    ...
    n_resp = len(coded_df)
    supplemental_test_count = int(include_valence) + int(include_emotion)
    supplemental_alpha = 0.05
    supplemental_alpha_note = "unadjusted per-test threshold; no cross-test correction"
    ...
    top_pct = round(theme_counts.iloc[0] / len(_themed_df) * 100, 1)
    cells.append(_md(
    ...
    # ── Theme distribution ─────────────────────────────────────
    show_all_tags = is_multi and theme_distribution_view == "all_tags"
    if show_all_tags:
        theme_intro = (
        ...
        theme_intro = (
            f"Chart shows the **primary theme only** (one theme per response). "
            f"Most common theme: **{top_theme}** ({top_pct}% of responses)."
            if is_multi else f"Most common theme: **{top_theme}** ({top_pct}% of responses)."
        )
        count_code = (
            "counts = df_analysis['primary_theme'].value_counts().reset_index()\n"
            "counts.columns = ['Theme', 'Count']\n"
            "_denom = len(df_analysis)  # % of responses assigned at least one theme\n"
            "_unit = 'Responses'\n"
        )
    ...
    cells.append(_code(
        ...
        "    xaxis={'title': f'Number of {_unit}', 'range': [0, counts['Count'].max() * 1.4]},\n"
    ...
    if covariate_cols:
        ...
        mc_note = (
            f"> **Multiple comparisons:** Using familywise α = 0.05 across {n_cov} covariate test(s), "
            f"the Bonferroni per-test threshold is **α = {bonf_thresh}**. Raw p-values are reported, "
            "and conclusions below compare them with this adjusted threshold."
        )
        cells.append(_md(
        ...
        for cov in covariate_cols:
            ...
            if cov_type == "categorical":
                try:
                    ...
                    threshold_note = (
                        f'below the Bonferroni-adjusted α = {bonf_thresh}'
                        if stats["p_value"] < bonf_thresh else
                        f'not below the Bonferroni-adjusted α = {bonf_thresh}'
                    )
                    stat_note = (
                        f'{stats["method"]}: statistic = {stats["chi2"]:.2f}, '
                        f'{p_clause}; {threshold_note} (familywise α = 0.05), bias-corrected Cramér\'s V = '
                ...
                cells.append(_code(
                    ...
                    f"_cat_labels = [_format_category_label(c) for c in _pct.index]\n"
                    ...
                    f"        name=theme, x=_pct.index.tolist(), y=_pct[theme].tolist(),\n"
                    ...
                    f"           'tickmode': 'array', 'tickvals': _pct.index.tolist(), 'ticktext': _cat_labels}},\n"
                    ...
                    f"_display_x = [_format_category_label(c) for c in _hct.index]\n"
                    f"fig2 = px.imshow(\n"
                    ...
                    f")\n"
                    f"fig2.update_xaxes(tickvals=list(range(len(_hct.index))), ticktext=_display_x, tickangle=-30)\n"
                    ...
                    f"_p_clause = f'p {{_p_text}}' if _p_text.startswith('<') else f'p = {{_p_text}}'\n"
                    f"print(f\"{{_assoc['method']}}: statistic={{_assoc['chi2']:.2f}}, "
                    f"{{_p_clause}}, df={{_assoc['dof']}}, bias-corrected Cramér's V={{_assoc['cramers_v']:.3f}}\")\n"
                    f"print('This p-value is below the Bonferroni-adjusted threshold α = {bonf_thresh} '"
                    f"      '(familywise α = 0.05).' if _assoc['p_value'] < {bonf_thresh} else "
                    f"      'This p-value is not below the Bonferroni-adjusted threshold α = {bonf_thresh} '"
                    f"      '(familywise α = 0.05).')\n"
                ...
                cells.append(_code(
                    ...
                    f"    _pc = f'p {{_pt}}' if _pt.startswith('<') else f'p = {{_pt}}'\n"
                    f"    print(f'Kruskal-Wallis: H={{_h:.2f}}, {{_pc}}, ε²={{_eps2:.3f}}; Bonferroni-adjusted threshold α = {bonf_thresh} (familywise α = 0.05)')\n"
                    f"    if _p < {bonf_thresh}:\n"
                    ...
                    f"        _ph['Below α=0.05'] = _ph['p-adj (Bonferroni)'].apply(lambda p: '✓' if p < 0.05 else '')\n"
                    f"        display(_ph)\n"
                    f"else:\n"
                    ...
                    f"    _pt = '< 0.001' if _p < 0.001 else f'{{_p:.3f}}'\n"
                    f"    _pc = f'p {{_pt}}' if _pt.startswith('<') else f'p = {{_pt}}'\n"
                    f"    print(f\"Welch's ANOVA: F={{_f:.2f}}, df={{_df1}},{{_df2:.1f}}, {{_pc}}, descriptive η²={{_eta2:.3f}}; Bonferroni-adjusted threshold α = {bonf_thresh} (familywise α = 0.05)\")\n"
                    f"    if _p < {bonf_thresh}:\n"
                    f"        _pairs = list(itertools.combinations(sorted(_groups), 2))\n"
                    f"        _rows = []\n"
                    f"        for a, b in _pairs:\n"
    ...
    if include_valence and "valence_score" in coded_df.columns:
        ...
        cells.append(_code(
            ...
            "_val_counts['Pct'] = _val_counts['Count'] / max(len(df_analysis), 1) * 100\n"
            "_val_counts['Label'] = _val_counts.apply(lambda r: f\"{int(r['Count'])} ({r['Pct']:.1f}%)\", axis=1)\n"
            "fig = go.Figure(go.Bar(\n"
            "    x=_val_counts['Count'], y=_val_counts['Valence'], orientation='h',\n"
            "    text=_val_counts['Label'], textposition='outside', textfont=dict(size=15), cliponaxis=False,\n"
            "    marker_color=[_val_colors.get(v, '#aaa') for v in _val_counts['Valence']],\n"
            "))\n"
            "fig.update_layout(\n"
            "    title='Valence Distribution', showlegend=False, bargap=0.3,\n"
            "    yaxis={'categoryorder': 'array', 'categoryarray': _val_order,\n"
            "           'automargin': True},\n"
            "    xaxis={'title': 'Number of Responses', 'range': [0, max(_val_counts['Count'].max() * 1.4, 1)]},\n"
        ...
        cells.append(_md(
            ...
            f"The threshold is α = {supplemental_alpha:.3g} "
            f"({supplemental_alpha_note})."
        ))
        cells.append(_code(
            ...
            "_vnorm = []\n"
            "for _name, _values in _vg.items():\n"
            "    _vp = shapiro(_values[:5000]).pvalue\n"
            "    _vnorm.append({'Theme': _name, 'N': len(_values),\n"
            "                   'p': '< 0.001' if _vp < 0.001 else f'{_vp:.3f}'})\n"
            "display(pd.DataFrame(_vnorm).assign(_sort=[shapiro(v[:5000]).pvalue for v in _vg.values()])\n"
            "        .sort_values('_sort').drop(columns='_sort').reset_index(drop=True))\n"
            "print('Theme groups above are ordered from smallest to largest Shapiro-Wilk p-value.')\n"
            "_vfail = any(shapiro(v[:5000]).pvalue < 0.05 for v in _vg.values())\n"
            "if _vfail:\n"
            ...
            "    _pc = f'p {_pt}' if _pt.startswith('<') else f'p = {_pt}'\n"
            f"    print(f'Kruskal-Wallis: H={{_h:.2f}}, {{_pc}}, ε²={{_eps2:.3f}}; threshold α = {supplemental_alpha:.3g}')\n"
            f"    if _p < {supplemental_alpha}:\n"
            ...
            "        _ph['Below α=0.05'] = _ph['p-adj (Bonferroni)'].apply(lambda p: '✓' if p < 0.05 else '')\n"
            "        display(_ph)\n"
            "else:\n"
            ...
            "    _pt = '< 0.001' if _p < 0.001 else f'{_p:.3f}'\n"
            "    _pc = f'p {_pt}' if _pt.startswith('<') else f'p = {_pt}'\n"
            f"    print(f\"Welch's ANOVA: F={{_f:.2f}}, df={{_df1}},{{_df2:.1f}}, {{_pc}}, descriptive η²={{_eta2:.3f}}; threshold α = {supplemental_alpha:.3g}\")\n"
            f"    if _p < {supplemental_alpha}:\n"
            "        _pairs = list(itertools.combinations(sorted(_vg), 2))\n"
            "        _rows = []\n"
            "        for a, b in _pairs:\n"
    ...
    if include_emotion and "emotion" in coded_df.columns:
        ...
        cells.append(_code(
            ...
            "_emo_counts['Pct'] = _emo_counts['Count'] / max(len(df_analysis), 1) * 100\n"
            "_emo_counts['Label'] = _emo_counts.apply(lambda r: f\"{int(r['Count'])} ({r['Pct']:.1f}%)\", axis=1)\n"
            "fig = go.Figure(go.Bar(\n"
            "    x=_emo_counts['Count'], y=_emo_counts['Emotion'], orientation='h',\n"
            "    text=_emo_counts['Label'], textposition='outside', textfont=dict(size=15), cliponaxis=False,\n"
            "    marker_color=[_emo_colors.get(e, '#aaa') for e in _emo_counts['Emotion']],\n"
            "))\n"
            "fig.update_layout(\n"
            "    title='Emotion Distribution', showlegend=False, bargap=0.3,\n"
            "    xaxis={'title': 'Number of Responses', 'range': [0, max(_emo_counts['Count'].max() * 1.4, 1)]},\n"
            "    yaxis={'automargin': True},\n"
        ...
        cells.append(_code(
            ...
            "_emo_pc = f'p {_emo_p}' if _emo_p.startswith('<') else f'p = {_emo_p}'\n"
            "print(f\"{_emo_assoc['method']}: statistic={_emo_assoc['chi2']:.2f}, "
            "df={_emo_assoc['dof']}, {_emo_pc}, bias-corrected Cramér's V={_emo_assoc['cramers_v']:.3f}\")\n"
            f"print('This p-value is below threshold α = {supplemental_alpha:.3g}.' "
            f"if _emo_assoc['p_value'] < {supplemental_alpha} else "
            f"'This p-value is not below threshold α = {supplemental_alpha:.3g}.')\n"
