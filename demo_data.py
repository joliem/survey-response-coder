...
# Earlier precomputation runs used shorter names for the same demo concepts.
# Translate them before checking the current/refined taxonomy so those rows do
# not silently fall back to keyword coding.
_DEMO_THEME_ALIASES = {
    "Identity Theft & Fraudulent Accounts": "Identity Theft and Fraudulent Accounts",
    "Dispute Process Failures": "Dispute and Investigation Process Issues",
    "Inaccurate Payment History & Account Data": "Inaccurate or Erroneous Credit Reporting",
    "Debt Collection Practices": "Debt Collection and Validation Issues",
    "Loan & Mortgage Servicing": "Loan Servicing and Payment Disputes",
    "Billing, Fees & Card Disputes": "Banking and Account Access Problems",
    "Account Restriction & Fund Access": "Banking and Account Access Problems",
}
...
def _taxonomy_from_seeds(user_seeds: str) -> list[dict]:
    ...
    for item in parsed:
        ...
        themes.append({
            ...
            "_source_names": [matched["name"]] if matched else [],
        })
...
def code_responses_demo(
    ...
    for theme in taxonomy:
        for source in theme.get("_source_names", [theme["name"]]):
            source_targets.setdefault(source, []).append(theme["name"])
    ...
    for i, text in enumerate(responses):
        ...
        if use_precomputed:
            ...
            source_name = _DEMO_THEME_ALIASES.get(_stored, _stored)
            translated = source_name if source_name in taxonomy_name_set else None
            if _stored == _NONE_THEME:
                translated = _NONE_THEME
            if translated is None and pd.notna(_stored):
                targets = source_targets.get(source_name, [])
                translated = targets[0] if len(targets) == 1 else None
            if pd.notna(_stored) and translated:
                primary = translated
        ...
        if multi_theme:
            ...
            if row_precomputed and has_secondary:
                ...
                secondary_source = _DEMO_THEME_ALIASES.get(secondary, secondary)
                translated_secondary = secondary_source if secondary_source in taxonomy_name_set else None
                if translated_secondary is None and pd.notna(secondary):
                    targets = source_targets.get(secondary_source, [])
                    translated_secondary = targets[0] if len(targets) == 1 else None
                if pd.notna(secondary) and translated_secondary and translated_secondary != primary:
                    themes.append(translated_secondary)
