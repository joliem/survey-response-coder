...
def theme_bar_chart(df: pd.DataFrame, theme_col: str, color_map: dict | None = None,
                    total: int | None = None, unit: str = "Responses") -> go.Figure:
...
def _format_category_label(text: str, max_chars: int = 40) -> str:
...
def sentiment_distribution_chart(df: pd.DataFrame, label_col: str) -> go.Figure:
    ...
    denom = max(len(df), 1)
    counts["Pct"] = counts["Count"] / denom * 100
    ...
    fig.update_layout(
        ...
        xaxis={"title": "Number of Responses", "range": [0, max(counts["Count"].max() * 1.4, 1)]},
...
def normality_summary(df: pd.DataFrame, theme_col: str, numeric_col: str) -> dict:
    ...
    for name, g in df.groupby(theme_col):
        ...
        rows.append({
            ...
            "N": n,
            "W": round(float(w), 4),
            # Keep the raw value for accurate ordering and downstream formatting.
            "p": float(p),
