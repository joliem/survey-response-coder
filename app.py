...
DEFAULTS = {
    ...
    "covariate_types": {},    # report-screen type choices, also used by the notebook
...
# ============================================================
    ...
    def _read_taxonomy(edited_df):
        ...
        original = st.session_state.taxonomy
        orig_lookup = {t["name"]: t for t in original}
        rows = edited_df.dropna(subset=["Theme Name"])
        rows = rows[rows["Theme Name"].str.strip() != ""]
        result = []
        for position, (_, row) in enumerate(rows.iterrows()):
            name = row["Theme Name"].strip()
            # A direct table rename retains the original theme identity by row position.
            prior = orig_lookup.get(name, original[position] if position < len(original) else {})
            result.append({
                "name": name,
                "description": str(row.get("Description") or ""),
                "examples": prior.get("examples", []),
                "_keywords": prior.get("_keywords", []),
                "_source_names": prior.get("_source_names", [prior["name"]] if prior.get("name") else []),
            })
        return result
                ...
                if len(to_merge) < 2:
                    ...
                    for t in current:
                        if t["name"] in to_merge:
                            ...
                            merged_sources.extend(t.get("_source_names", [t["name"]]))
                    ...
                    new_taxonomy.append({
                        ...
                        "_source_names": list(dict.fromkeys(merged_sources)),
                    })
                        ...
                        new_taxonomy.extend([
                            {
                                ...
                                "_keywords": kws_1,
                                "_source_names": old_theme.get("_source_names", [to_split]) if old_theme else [to_split],
                            },
                            {
...
# ============================================================
    ...
    if st.session_state.coded_df is None:
        ...
        if _start:
            ...
            # Normalize + validate themes (skip non-strings / JSON artifacts); single-theme enforce.
            _valid_themes = {t["name"] for t in taxonomy}
            for _i, _r in enumerate(results):
                if not isinstance(_r, dict):
                    _r = {}
                    results[_i] = _r
                _kept = [th for th in (_r.get("themes") or []) if isinstance(th, str) and th in _valid_themes]
                if not multi_theme:
                    _kept = _kept[:1]
    ...
    if st.session_state.coded_df is not None:
        ...
        is_multi_theme = bool(st.session_state.get("multi_theme", False))
        show_cols = (
...
# ============================================================
    ...
    is_multi = bool(st.session_state.get("multi_theme", False))
    ...
    if is_multi:
        st.info(
            f"Multi-theme coding assigned {len(analysis_df):,} theme tags across "
            f"{len(themed_df):,} themed responses. Covariate analysis always uses the **primary theme** "
            f"(most relevant) per response to preserve statistical independence.",
            icon="ℹ️",
        )
        _theme_distribution_view = st.radio(
            "Show",
            options=["All Theme Tags", "Primary Themes Only"],
            horizontal=True,
            key="theme_distribution_view",
        )
    else:
        _theme_distribution_view = "Primary Themes Only"

    _show_all_tags = is_multi and _theme_distribution_view == "All Theme Tags"
    _distribution_df = analysis_df if _show_all_tags else themed_df
    _distribution_col = "theme" if _show_all_tags else "primary_theme"
    _chart_total = len(analysis_df) if _show_all_tags else len(themed_df)
    _chart_unit = "Theme Tags" if _show_all_tags else "Responses"
    # Set the dynamic title on the returned figure instead of requiring a new
    # helper argument. This remains safe during Streamlit Cloud hot reloads,
    # where app.py can refresh before an already-imported analysis.py module.
    _theme_distribution_fig = theme_bar_chart(
        _distribution_df, _distribution_col, color_map=_theme_color_map,
        total=_chart_total,
    )
    _theme_distribution_fig.update_xaxes(title_text=f"Number of {_chart_unit}")
    st.plotly_chart(
        _theme_distribution_fig,
        use_container_width=True,
    )
    if _show_all_tags:
        st.caption(
        ...
        st.caption(
            f"Each bar shows the response count and its share of the {len(themed_df):,} responses "
            "assigned at least one theme. Blank and 'None of the above' responses are excluded — see below."
        )
    ...
    if covariate_cols:
        ...
        # Control the familywise Type-I error rate across the selected covariate
        # omnibus tests with a Bonferroni per-test threshold.
        _family_alpha = 0.05
        _omnibus_alpha = _family_alpha / max(1, len(covariate_cols))
        ...
        cov_type = st.radio(
            ...
            index=["categorical", "numeric", "date"].index(
                st.session_state.covariate_types.get(selected_cov, detected)
            ),
            horizontal=True,
            key=f"covariate_type_{selected_cov}",
        )
        st.session_state.covariate_types[selected_cov] = cov_type
        ...
        if cov_type == "categorical":
            ...
            st.subheader(f"Association Test: Theme × {selected_cov}")
            # Test the complete covariate, even when charts show only the top categories.
            stats = chi_square_summary(covariate_df, "primary_theme", selected_cov)
            _p_display = format_p_value(stats["p_value"])
            _p_clause = f"p {_p_display}" if _p_display.startswith("<") else f"p = {_p_display}"
            _below_threshold = stats["p_value"] < _omnibus_alpha
            m1, m2, m3, m4 = st.columns(4)
            m1.metric("Chi² Statistic", f'{stats["chi2"]:.2f}')
            m2.metric("p-value", _p_display)
            ...
            if _below_threshold:
                ...
                st.success(
                    f"Association between themes and {selected_cov} ({_p_clause}); the p-value is below "
                    f"the Bonferroni-adjusted threshold of α = {_omnibus_alpha:.3g} "
                    f"(familywise α = {_family_alpha:.2f} across {len(covariate_cols)} covariate tests). "
                    f"Bias-corrected Cramér's V = {stats['cramers_v']} — {strength} effect."
                )
            else:
                st.info(
                    f"The association test produced {_p_clause}, which is not below the "
                    f"Bonferroni-adjusted threshold of α = {_omnibus_alpha:.3g}."
                )
            ...
            if n_cov > 1:
                ...
                st.caption(
                    ...
                    f"across groups of the covariate. **Method: {stats['method']}.** "
                    f"If theme and {selected_cov} were unrelated, the smallest expected table-cell count would be "
                    f"{stats['expected_min']:.2f}; {stats['expected_below_5_pct']:.1f}% of table cells would have "
                    "expected counts below 5. These are expected cell counts, not observed category totals. "
                    "When the expected-count assumptions fail, the reported p-value comes from label permutations. "
                    ...
                    f"The app uses the Bonferroni per-test threshold **α = {bonf_thresh}** to maintain "
                    "a familywise α = 0.05 across the selected covariate tests."
                )
            else:
                st.caption(
                    "**Chi-square test of independence** asks whether the distribution of themes differs "
        ...
        # ── Date / Time ─────────────────────────────────────────
            ...
            if trend_df.empty:
                ...
                def _trend_flag(row):
                    if row["Below α=0.05"] == "✓" and row["Slope (pp/period)"] > 0:
                        return "🟢"
                    if row["Below α=0.05"] == "✓" and row["Slope (pp/period)"] < 0:
                        return "🔴"
                    return ""
                ...
                sig_rising = trend_df[(trend_df["Below α=0.05"] == "✓") & (trend_df["Slope (pp/period)"] > 0)]
                sig_falling = trend_df[(trend_df["Below α=0.05"] == "✓") & (trend_df["Slope (pp/period)"] < 0)]
                if not sig_rising.empty:
                    rising_names = ", ".join(f"**{t}**" for t in sig_rising["Theme"])
                    st.success(f"Rising trends with Holm-adjusted p below α = 0.05: {rising_names}")
                if not sig_falling.empty:
                    falling_names = ", ".join(f"**{t}**" for t in sig_falling["Theme"])
                    st.info(f"Falling trends with Holm-adjusted p below α = 0.05: {falling_names}")
                if sig_rising.empty and sig_falling.empty:
                    st.info("No theme has a Holm-adjusted trend-test p-value below α = 0.05.")
                st.caption(
                    ...
                    "**p-value** measures compatibility with a zero-slope model. "
                    "Rows are ordered by theme frequency (most common first); "
                    "🟢 = rising and Holm-adjusted p below α = 0.05; 🔴 = falling and below that threshold. "
                    "⚠️ Treat as exploratory: time series data can have autocorrelation that inflates "
                    "false-positive rates, and short windows may reflect noise rather than real trends."
                )
        ...
        # ── Numeric / Continuous ─────────────────────────────────
            ...
            with st.expander("Normality check (Shapiro-Wilk per group)"):
                norm_df = pd.DataFrame(norm["groups"]).sort_values("p", na_position="last")
                norm_df["p"] = norm_df["p"].map(
                    lambda value: format_p_value(value) if pd.notna(value) else "—"
                )
                ...
                st.caption(
                    ...
                    "assumption doesn't hold. Testable groups are ordered from smallest to largest p-value."
                )
            ...
            if use_kruskal:
                ...
                if stats is None:
                    ...
                    m1.metric("H Statistic", f'{stats["h_stat"]:.2f}')
                    m2.metric("p-value", format_p_value(stats["p_value"]))
                    m3.metric("ε² (Epsilon-squared)", stats["epsilon_squared"])
                    if stats["p_value"] < _omnibus_alpha:
                        strength = (
                        ...
                        st.success(
                            f"Difference in **{selected_cov}** across themes; the p-value is below "
                            f"the Bonferroni-adjusted threshold α = {_omnibus_alpha:.3g} "
                            f"(H = {stats['h_stat']:.2f}, p {format_p_value(stats['p_value']) if format_p_value(stats['p_value']).startswith('<') else '= ' + format_p_value(stats['p_value'])}). "
                            f"ε² = {stats['epsilon_squared']:.3f} — {strength} effect size."
                        )
                    else:
                        st.info(
                            f"For {selected_cov}, p = {format_p_value(stats['p_value'])}, which is not below "
                            f"the Bonferroni-adjusted threshold α = {_omnibus_alpha:.3g}."
                        )
                    st.caption(
                    ...
                    if stats["p_value"] < _omnibus_alpha:
                        with st.expander("Post-hoc pairwise comparisons"):
                            st.markdown(
                                ...
                                "pairs account for the omnibus result that crossed the stated threshold."
                            )
                            ...
                            if not ph.empty:
                                ...
                                st.caption(
                                    ...
                                    f"{len(ph)} pairwise comparisons. **Below α ✓** means adjusted p < 0.05."
                                )
            else:
                st.subheader(f"Welch's ANOVA: {selected_cov} × Theme")
                stats = anova_summary(covariate_df, "primary_theme", selected_cov)
                if stats is None:
                    st.warning("Not enough data per group to run ANOVA.")
                else:
                    m1, m2, m3 = st.columns(3)
                    m1.metric("F Statistic", f'{stats["f_stat"]:.2f}')
                    m2.metric("p-value", format_p_value(stats["p_value"]))
                    ...
                    if stats["p_value"] < _omnibus_alpha:
                        ...
                        st.success(
                            f"Difference in **{selected_cov}** across themes; the p-value is below "
                            f"the Bonferroni-adjusted threshold α = {_omnibus_alpha:.3g} "
                            f"(F = {stats['f_stat']:.2f}, p {format_p_value(stats['p_value']) if format_p_value(stats['p_value']).startswith('<') else '= ' + format_p_value(stats['p_value'])}). "
                            f"η² = {stats['eta_squared']:.3f} — {strength} effect size."
                        )
                    else:
                        st.info(
                            f"For {selected_cov}, p = {format_p_value(stats['p_value'])}, which is not below "
                    ...
                    st.caption(
                        "**Welch's ANOVA** tests whether the mean value differs across theme groups without "
                        "assuming equal variances. **η² (eta-squared)** is shown descriptively as the proportion of "
                    ...
                    if stats["p_value"] < _omnibus_alpha:
                        with st.expander("Post-hoc pairwise comparisons"):
                            st.markdown(
                                ...
                                "account for the omnibus result that crossed the stated threshold."
                            )
                            ...
                            if not ph.empty:
                                ...
                                st.caption(
                                    ...
                                    "**Below α ✓** means the adjusted p < 0.05."
                                )

    # Valence and emotion are closely related descriptive endpoints. Report each
    # against its own unadjusted threshold; post-hoc comparisons remain adjusted.
    _supplemental_test_count = int("valence_score" in covariate_df.columns) + int("emotion" in covariate_df.columns)
    _supplemental_alpha = 0.05
    _supplemental_alpha_explanation = "unadjusted per-test threshold; no cross-test correction"
    ...
    if "valence_score" in covariate_df.columns:
        ...
        with st.expander("Statistical Test: Valence Score × Primary Theme"):
            ...
            _val_test_name = "Kruskal-Wallis" if _val_use_kruskal else "Welch's ANOVA"
            st.markdown(
                f"Testing whether **valence scores differ across themes** using **{_val_test_name}** "
                f"(chosen based on the normality check below). The threshold is "
                f"α = {_supplemental_alpha:.3g} ({_supplemental_alpha_explanation})."
            )

            with st.expander("Normality check (Shapiro-Wilk per theme group)"):
                _val_norm_df = pd.DataFrame(_val_norm["groups"]).sort_values("p", na_position="last")
                _val_norm_df["p"] = _val_norm_df["p"].map(
                    lambda value: format_p_value(value) if pd.notna(value) else "—"
                )
                ...
                if _val_norm["any_fail"]:
                    ...
                    st.success("All groups appear normally distributed — using Welch's ANOVA.")
                st.caption(
                    "p-values are shown to three decimal places (values below 0.001 are shown as < 0.001). "
                    "Testable theme groups are ordered from smallest to largest p-value."
                )
            ...
            if _val_use_kruskal:
                ...
                if _val_stats is None:
                    ...
                    _vm1.metric("H Statistic", f'{_val_stats["h_stat"]:.2f}')
                    _vm2.metric("p-value", format_p_value(_val_stats["p_value"]))
                    _vm3.metric("ε² (Epsilon-squared)", _val_stats["epsilon_squared"])
                    if _val_stats["p_value"] < _supplemental_alpha:
                        _val_strength = (
                        ...
                        st.success(
                            f"Valence difference with p below α = {_supplemental_alpha:.3g} "
                            f"(H = {_val_stats['h_stat']:.2f}, p {format_p_value(_val_stats['p_value']) if format_p_value(_val_stats['p_value']).startswith('<') else '= ' + format_p_value(_val_stats['p_value'])}). "
                            f"ε² = {_val_stats['epsilon_squared']:.3f} — {_val_strength} effect."
                        )
                        with st.expander("Post-hoc pairwise comparisons (Bonferroni-corrected Mann-Whitney U)"):
                            ...
                            if not _val_ph.empty:
                                ...
                                st.caption(
                                    ...
                                    "**Below α ✓** means adjusted p < 0.05."
                                )
                    else:
                        st.info(
                            f"The valence-test p-value is not below α = {_supplemental_alpha:.3g} "
                            f"(p = {format_p_value(_val_stats['p_value'])})."
                        )
                    st.caption(
                ...
                if _val_stats is None:
                    ...
                    _vm1, _vm2, _vm3 = st.columns(3)
                    _vm1.metric("F Statistic", f'{_val_stats["f_stat"]:.2f}')
                    _vm2.metric("p-value", format_p_value(_val_stats["p_value"]))
                    ...
                    if _val_stats["p_value"] < _supplemental_alpha:
                        ...
                        st.success(
                            f"Valence difference with p below α = {_supplemental_alpha:.3g} "
                            f"(F = {_val_stats['f_stat']:.2f}, p {format_p_value(_val_stats['p_value']) if format_p_value(_val_stats['p_value']).startswith('<') else '= ' + format_p_value(_val_stats['p_value'])}). "
                            f"η² = {_val_stats['eta_squared']:.3f} — {_val_strength} effect."
                        )
                    ...
                    st.caption(
                        "**Welch's ANOVA** tests whether mean valence differs without assuming equal variances. "
                        "**η²** is shown descriptively as the proportion of variance explained by theme: "
    ...
    if "emotion" in covariate_df.columns:
        ...
        with st.expander("Statistical Test: Emotion Label × Primary Theme"):
            ...
            if len(_emo_clean) < 10 or _emo_clean["emotion"].nunique() < 2:
                ...
                _emo_p_display = format_p_value(_emo_stats["p_value"])
                st.markdown(
                    "Testing whether **emotion label distributions differ across themes** "
                    f"using **{_emo_stats['method']}**. The threshold is "
                    f"α = {_supplemental_alpha:.3g} ({_supplemental_alpha_explanation})."
                )
                _em1, _em2, _em3, _em4 = st.columns(4)
                _em1.metric("Chi² Statistic", f'{_emo_stats["chi2"]:.2f}')
                _em2.metric("p-value", format_p_value(_emo_stats["p_value"]))
                _em3.metric("Degrees of Freedom", _emo_stats["dof"])
                _em4.metric("Cramér's V", _emo_stats["cramers_v"])
                if _emo_stats["p_value"] < _supplemental_alpha:
                    _emo_strength = (
                    ...
                    st.success(
                        f"Emotion–theme association with p below α = {_supplemental_alpha:.3g} "
                        f"(p {_emo_p_display if _emo_p_display.startswith('<') else '= ' + _emo_p_display}). "
                        f"Bias-corrected Cramér's V = {_emo_stats['cramers_v']} — {_emo_strength} effect."
                    )
                else:
                    st.info(
                        f"The emotion–theme association p-value is not below α = {_supplemental_alpha:.3g} "
                        f"(p = {_emo_p_display})."
                    )
                st.caption(
                    ...
                    "If emotion and primary theme were unrelated, the smallest expected table-cell count "
                    f"would be {_emo_stats['expected_min']:.2f}; {_emo_stats['expected_below_5_pct']:.1f}% of "
                    "table cells would have expected counts below 5. These are expected cell counts, not "
                    "observed category totals. When the expected-count assumptions fail, the displayed "
                    "p-value is obtained by permutation. "
                    "**Cramér's V** measures effect size on a 0–1 scale: "
                    "< 0.1 = negligible, 0.1–0.3 = moderate, > 0.3 = strong. "
    ...
    _is_multi = bool(st.session_state.get("multi_theme", False))
    _quotes_ready = bool(st.session_state.theme_quotes)
    _download_df = prepare_download_dataframe(
        coded_df, _is_multi, st.session_state.theme_quotes or None
    )
    ...
    with _dl_col2:
        ...
        _nb_bytes = generate_notebook(
            ...
            theme_distribution_view=("all_tags" if _show_all_tags else "primary_themes"),
        )
        if st.download_button(
