...
# ============================================================
    ...
    # Valence and emotion are a separate family of supplemental omnibus tests.
    _supplemental_test_count = int("valence_score" in covariate_df.columns) + int("emotion" in covariate_df.columns)
    _supplemental_alpha = 0.05 / max(1, _supplemental_test_count)
    ...
    if "valence_score" in covariate_df.columns:
        ...
        with st.expander("Statistical Test: Valence Score × Primary Theme"):
            ...
            st.markdown(
                ...
                f"(chosen based on the normality check below). The Bonferroni-adjusted threshold is "
                f"α = {_supplemental_alpha:.3g} (familywise α = 0.05 across "
                f"{_supplemental_test_count} supplemental test(s))."
            )
        ...
        with st.expander("Statistical Test: Emotion Label × Primary Theme"):
            ...
            if len(_emo_clean) < 10 or _emo_clean["emotion"].nunique() < 2:
                ...
                st.markdown(
                    ...
                    f"using **{_emo_stats['method']}**. The Bonferroni-adjusted threshold is "
                    f"α = {_supplemental_alpha:.3g} (familywise α = 0.05 across "
                    f"{_supplemental_test_count} supplemental test(s))."
                )
