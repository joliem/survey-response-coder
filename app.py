    ...
    if covariate_cols:
        ...
        if cov_type == "categorical":
            ...
            if _below_threshold:
                ...
                st.success(
                    ...
                    f"the Bonferroni-adjusted threshold of α = {_omnibus_alpha:.3f} "
                ...
                st.info(
                    ...
                    f"Bonferroni-adjusted threshold of α = {_omnibus_alpha:.3f}."
                )
        ...
        # ── Date / Time ─────────────────────────────────────────
            ...
            if trend_df.empty:
                ...
                def _trend_flag(row):
                    if row["Below α=.05"] == "✓" and row["Slope (pp/period)"] > 0:
                        return "🟢"
                    if row["Below α=.05"] == "✓" and row["Slope (pp/period)"] < 0:
                        return "🔴"
                    return ""
                ...
                sig_rising = trend_df[(trend_df["Below α=.05"] == "✓") & (trend_df["Slope (pp/period)"] > 0)]
                sig_falling = trend_df[(trend_df["Below α=.05"] == "✓") & (trend_df["Slope (pp/period)"] < 0)]
                ...
                if stats is None:
                    ...
                    if stats["p_value"] < _omnibus_alpha:
                        ...
                        st.success(
                            ...
                            f"the Bonferroni-adjusted threshold α = {_omnibus_alpha:.3f} "
    ...
    if "valence_score" in covariate_df.columns:
        ...
        with st.expander("Statistical Test: Valence Score × Primary Theme"):
            ...
            st.markdown(
                ...
                f"α = {_supplemental_alpha:.3f} (familywise α = 0.05 across "
            ...
            if _val_use_kruskal:
                ...
                if _val_stats is None:
                    ...
                    if _val_stats["p_value"] < _supplemental_alpha:
                        ...
                        st.success(
                            f"Valence difference with p below α = {_supplemental_alpha:.3f} "
                        ...
                        st.info(
                            f"The valence-test p-value is not below α = {_supplemental_alpha:.3f} "
        ...
        with st.expander("Statistical Test: Emotion Label × Primary Theme"):
            ...
            if len(_emo_clean) < 10 or _emo_clean["emotion"].nunique() < 2:
                ...
                if _emo_stats["p_value"] < _supplemental_alpha:
                    ...
                    st.success(
                        f"Emotion–theme association with p below α = {_supplemental_alpha:.3f} "
                    ...
                    st.info(
                        f"The emotion–theme association p-value is not below α = {_supplemental_alpha:.3f} "
                        f"(p = {_emo_p_display})."
                    )
                st.caption(
